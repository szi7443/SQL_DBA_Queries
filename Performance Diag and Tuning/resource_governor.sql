/*
How to configure resource governor in SQL enterprise through T-SQL
*/
...
/* Create classifier function deciding what workload group will be assigned to whom */

CREATE FUNCTION dbo.ResourceGov_Classifier() RETURNS sysname
WITH SCHEMABINDING
AS 
	BEGIN
...
ALTER RESROUCE GOVERNOR WITH ( CLASSIFIER FUNCTION = dbo.ResourceGov_Classifier);
	GO
/*install new config for RG*/
ALTER RESOURCE GOVERNOR RECONFIGURE;
GO

/* To create resource pools (defining resources available) */
...
/* Assign resource pool to workload group*/
ALTER WORKLOAD GROUP GroupAdmin USING PoolAdmin;
ALTER WORKLOAD GROUP GroupETL USING PoolETL;



/********************** template from SSMS for more options **********************/
